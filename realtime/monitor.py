...
iso_scalar = joblib.load("../data/iso_scaler.pkl")
...
def monitor(input_data, actual_data):
    ...
    actual_scaled=iso_scalar.transform(actual_df)
    # Isolation Forest checks all 3 metrics for spikes
    anomaly_label = iso_forest.predict(actual_scaled)   # -1 = anomaly, 1 = normal
    anomaly_score = iso_forest.decision_function(actual_scaled)  # lower = more anomalous
